...
class AdminController extends Controller{
		...
		$this->middleware(function ($request, $next){
			...
			if(Session::has('role')){
				...
				$this->pub = array(
					...
					'logo'   => $logo->value,
					//Sidebar Reports
					'applicants_reports' =>  DisplayModel::getAllApplicants(),
					'employee_reports' =>  DisplayModel::getAllEmployees()
	...
	public function processEditProfile(){
		...
		//$data['update_emp_id'] = updateModel::updateEmployeeIDViaID($personal_info_id,$employee_id);
	...
	public function processEditStatutories(){
		extract($_POST);
		$data['update_statutories'] = UpdateModel::updateStatutoriesViaID($personal_info_id,$sss,$pagibig,$philhealth,$tin);
		//$data['update_first_day'] = UpdateModel::updateFirstDayViaID($personal_info_id,$first_day);
		Session::flash('success', 'Statutories updated successfully!');
		return redirect()->back();	
	}
	//End Profile Module
	...
	public function processAddEmployee(){
		...
		$data['insert_to_personal_info'] = 	insertModel::insertToPersonalInfo($lname,$fname,$mname,$email,$address,$contact,$bday,$cstatus,$user_role,$employment_status,$gender,$department,$job_title,$salary);
		...
		$data['add_statutories'] = updateModel::updateStatutoriesViaID($pid->personal_info_id,$sss,$pagibig,$philhealth,$tin);	
		$data['add_first_day'] = updateModel::updateFirstDayViaID($pid->personal_info_id,$first_day);
		$data['add_emp_id'] = updateModel::updateEmployeeIDViaID($pid->personal_info_id,$employee_id);
	...
	public function processEditEmployee(){
		...
		if(isset($sss) && isset($pagibig) && isset($philhealth) && isset($tin)){
			$data['update_statutories'] = UpdateModel::updateStatutoriesViaID($personal_info_id,$sss,$pagibig,$philhealth,$tin);
		}
		...
		if(isset($feedback)){ 
			$data['insert_feed_back'] = UpdateModel::UpdateFeedbackViaID($personal_info_id,addslashes($feedback));
		}
	...
	public function editApplicant($id){
		...
		$data['employment'] = DisplayModel::getEmploymentBackgroundViaID($id);
		$data['files'] = DisplayModel::getAllUserFiles($id);
		$data['tests'] = DisplayModel::getTestsViaID($id);
		$data['reference'] = DisplayModel::getReferenceViaID($id);
	...
	public function debug(){	 
		...
		// $helper->sendEmail($recipient->value,"TEST EMAIL WITHOUT CC");
		// $helper->sendEmailWithCC($recipient->value,"TEST EMAIL WITH CC");
	}	
}